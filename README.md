...
Then keep this terminal running and open a new one for executing the frontend. From the root directory, head to Client/. 
There, run these commands in order to first install the frontend dependencies and then run the React app:
...
4. If you want to run the automated tests, execute the command below in a terminal in the root directory of this project while API is not running. 
This will run all the tests together. The tests cover amount validation, conversion results, and API request/response behavior. 
...
You can put in a value and click 'Convert To Words' or press Enter while in the text field. If the value is valid, the result would show in the result section. 
Press Clear to clear out the text field, any error messages that might've shown and the result section.  
If you edit the text field after a result is shown, it will automatically reset the result section.

If you put in an invalid value or the react app can't connect to the server, it will show the corresponding error message. 
Invalid values include letters, zero, negative amounts, grouping commas, scientific notation and a trailing decimal point such as '23.' . 

Valid values are:
- Positive numbers from 0.01 to 18446744073709551615.99.
- Whole numbers and numbers with up to two decimal places.
- Additional trailing fractional zeros, such as 1.2300.
- numbers below one, such as .25.
- Leading and trailing whitespace is trimmed.


If you want to interact with the app without using the UI, you can simply send an http request like the examples below with an app like Postman or use the Swagger UI. 

Swagger UI has the advantage of displaying the available API endpoints and their inputs. It also allows you to test requests directly from the browser. To use Swagger UI, go to the address 'https://localhost:5001/swagger' on your browser.
...
Missing or null input also returns 400. Malformed JSON and values that can't be read as a decimal return 400. In local tests in development, they returned exception details as plain text. 
When using the UI, it's handled by showing a default error message “The number could not be converted." if the frontend can't read a specific error message (if it's not in json format). 

The API receives a JSON number and reads it as a C# decimal. 
The frontend keeps the entered value as text when preparing the request which allows us to preserve large numbers without converting them to a JavaScript number. 
But direct requests must follow JSON number syntax: use 0.25 instead of .25, and 1.25 instead of 001.25.
